...
export default function ProductSalesReportPage() {
  ...
  const salesTrendData = useMemo(() => {
    ...
    const finalValues = filtered.map(f => f.value);
    return {
      labels: finalLabels,
      datasets: [
        {
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(20);
    doc.text('Product Sales Report', 20, yPosition);
    yPosition += 20;
  ...
  const exportToExcel = () => {
    ...
    const productsData = [
      ...
      ...(metrics.topProducts || []).map(p => [
        ...
        p.unitsSold,
        p.revenue,
        (p.revenue / p.unitsSold).toFixed(2),
        ...
        <header className="mb-10">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-lg text-gray-500">Detailed sales performance for each product including revenue, units sold, and trends.</p>
            </div>
              ...
              <table className="min-w-full">
                ...
                <tbody>
                  {(metrics.topProducts || []).map((product) => {
                    const avgPrice = product.revenue / product.unitsSold;
